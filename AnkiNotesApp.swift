...
final class AppState: ObservableObject {
    ...
    func applyCurrentProviderSelection(allowWebDAVIncomplete: Bool) -> Bool {
        ...
        switch type {
        case .local, .iCloud:
            applyFileSystem(type: type, webDAVConfig: webDAVConfig)
            iCloudContainerAvailable = (activeFS as? ICloudFS)?.isAvailable ?? false
            providerStatus = summarizeStatus()
            return true
        ...
        case .webDAV:
            ...
            guard complete else {
                ...
                if allowWebDAVIncomplete {
                    ...
                    🆕 先把 Picker 留在 WebDAV → 请在下方把缺少的项填好 → 再点「💾保存并应用 WebDAV 配置」按钮即可生效。
            ...
            // 3) 齐全 → 持久化配置 + 重建 Storage/Scheduler 服务（不迁移数据，用户下拉同步时拉取）
            Self.saveWebDAVConfigToDefaults(webDAVConfig)
            applyFileSystem(type: .webDAV, webDAVConfig: webDAVConfig)
    ...
    // MARK: - 内部：创建 CloudFileSystem + 重建 Storage/Scheduler
    // 注意：本地优先架构，StorageService 始终用 LocalFS，WebDAV 仅作为同步目标
    // 切换存储不迁移数据，用户在笔记页下拉同步时才从云端拉取数据

    private func applyFileSystem(type: CloudProviderType, webDAVConfig: WebDAVConfig) {
        let newFS = CloudProviderFactory.makeFileSystem(for: type, webDAVConfig: webDAVConfig)

        // 不做数据迁移：本地优先架构下，切换只是改变活跃的同步后端
        // 数据同步通过显式的下拉同步操作完成，避免切换时卡死和重复推送
